...
export function ArchivedLoansDrawer() {
    const { 
        isArchivedLoansListOpen, closeArchivedLoansList, openLoanRecords,
    ...
    return (
        <Sheet open={isArchivedLoansListOpen} onOpenChange={(open) => !open && closeArchivedLoansList()}>
            <SheetContent 
                side="bottom" 
                className={cn(
                ...
                <div className="flex-1 overflow-y-auto px-6 pb-12 relative z-10" data-scroll-container>
                    <SheetHeader className="px-0 py-4 shrink-0 border-b mb-6">
                        <div>
                            <SheetTitle className="text-2xl font-black tracking-tight flex items-center gap-1.5">
                                {t('archivedLoans')}
                                <DevBadge id="d:archived-loans" />
                            </SheetTitle>
                            <SheetDescription className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 mt-1 m-0 leading-none">
                                {loans.length} {t('totalLoans')}
                            </SheetDescription>
                        </div>
                    </SheetHeader>
                    ...
                    {loans.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 gap-4">
                            ...
                            <div>
                                <h3 className="font-bold text-lg mb-1">{t('noArchivedLoans')}</h3>
                            </div>
                        </div>
                    ) : (
                                ...
                                {loans.map(loan => (
                                    <div key={loan.id} className="flex items-center gap-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={() => archiveLoan(loan.id!, false)}
                                            title={t('unarchive')}
                                        >
